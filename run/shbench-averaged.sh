...
if [[ $# -ne 2 ]]; then
  echo "usage: shbench-averaged.sh <allocator name> <preload>"
  echo ""
  echo "averaged a fixed configuration of the shbench benchmark"
  echo ""
  echo "output directory: data/shbench/"
  echo "parameters:"
...
PRUNE="yes"
BINARY="./run/shbench-single.sh"
COMMAND="$BINARY "$PRELOAD" <threads>"

DATA_DIR=data/shbench
mkdir -p $DATA_DIR
