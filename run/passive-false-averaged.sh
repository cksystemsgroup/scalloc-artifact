...
if [[ $# -ne 2 ]]; then
  echo "usage: passive-false-averaged.sh <allocator name> <preload>"
  echo ""
  echo "averaged a fixed configuration of the passive-false benchmark"
  echo ""
  echo "output directory: data/passive-false/"
  echo "parameters:"
...
CFG="1000 8 1000000"
PRUNE="yes"
BINARY="./run/passive-false-single.sh"
COMMAND="$BINARY "$PRELOAD" <threads> \"$CFG\""

DATA_DIR=data/passive-false
mkdir -p $DATA_DIR
