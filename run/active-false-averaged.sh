...
if [[ $# -ne 2 ]]; then
  echo "usage: active-false-averaged.sh <allocator name> <preload>"
  echo ""
  echo "averaged a fixed configuration of the active-false benchmark"
  echo ""
  echo "output directory: data/active-false/"
  echo "parameters:"
...
# override THREAD_ITERATOR, since false sharing manifests already with low
# numbers of threads
...
CFG="1000 8 1000000"
PRUNE="yes"
BINARY="./run/active-false-single.sh"
COMMAND="$BINARY "$PRELOAD" <threads> \"$CFG\""

DATA_DIR=data/active-false
mkdir -p $DATA_DIR
